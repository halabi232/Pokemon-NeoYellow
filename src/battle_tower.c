...
const u16 gBattleFrontierHeldItems[] =
{
    ...
    [BATTLE_FRONTIER_ITEM_RAZOR_FANG]      = ITEM_RAZOR_FANG, 
    [BATTLE_FRONTIER_ITEM_RAZOR_CLAW]      = ITEM_RAZOR_CLAW,  
    ...
    [BATTLE_FRONTIER_ITEM_SMOOTH_ROCK]     = ITEM_SMOOTH_ROCK, 
...
static void FillTrainerParty(u16 trainerId, u8 firstMonId, u8 monCount)
{
    ...
    while (i != monCount)
    {
        ...
        SetMonData(&gEnemyParty[i + firstMonId], MON_DATA_ABILITY_NUM, &abilityNum);
...
u8 UNUSED lvlMode = gSaveBlock2Ptr->frontier.lvlMode;
    ...
    for (i = 0; i < FRONTIER_PARTY_SIZE; i++)
    {
        ...
        SetMonData(&gEnemyParty[firstMonId + i], MON_DATA_ABILITY_NUM, &abilityNum);
        SetMonData(&gEnemyParty[firstMonId + i], MON_DATA_FRIENDSHIP, &friendship);
            ...
            if (partyData[i].ability != ABILITY_NONE)
            {
                ...
                if (partyData[i].ability < maxAbilities)
                    if (speciesInfo->abilities[partyData[i].ability] != ABILITY_NONE)
                        SetMonData(&gPlayerParty[i + 3], MON_DATA_ABILITY_NUM, &speciesInfo->abilities[partyData[i].ability]);
            }
